...
[bold yellow]Warning[/bold yellow]: Cases in lhs and rhs are not equal.
    ...
    * lhs cases: {', '.join(set(lhs['cases'].keys()) - slugs)}.
            ...
            if "entries" in data_json and cb_test in data_json["entries"]:
                ...
                for case_name, values in case_times.items():
                    if len(values) > 0:
                        ...
                        cb_stats[case_name] = {"avg": avg, "std_dev": std_dev, "count": len(values)}
    ...
    if cb_stats:
        table.add_column("[bold] Grind vs CB[/bold]", justify="right")
            ...
            try:
                ...
                if target == SIMULATION:
                    ...
                    if slug in cb_stats:
                        ...
                        if std_dev > 0:
                            deviation_sigma = (rhs_grind - avg) / std_dev
                            color = "green" if abs(deviation_sigma) <= 1 else "yellow" if abs(deviation_sigma) <= 2 else "red"
                            grind_comparison = f"[{color}]{deviation_sigma:+.2f}σ[/{color}] (avg: {avg:.0f}±{std_dev:.0f})"
                        else:
                            deviation_pct = ((rhs_grind - avg) / avg * 100) if avg > 0 else 0
                            color = "green" if abs(deviation_pct) <= 5 else "yellow" if abs(deviation_pct) <= 10 else "red"
                            grind_comparison = f"[{color}]{deviation_pct:+.1f}%[/{color}] (avg: {avg:.0f})"
